...
import { Scissors, ShoppingCart, Menu, X, LogIn, Trash2, Plus, Minus, User } from "lucide-react"
...
export default function Navbar({ cart = [], onUpdateCart }: NavbarProps) {
...
_Orden generada desde la tienda online_`
  ...
  return (
    <nav className="fixed top-0 w-full bg-elementz-dark z-50 border-b border-elementz-brown">
            ...
            {user ? (
              <div className="flex items-center space-x-4">
                <button
                  ...
                  className="text-elementz-gray transition-colors font-lato flex items-center"
                >
                ...
                <Button
                  ...
                  className="bg-elementz-brown text-elementz-cream hover:bg-elementz-brown/80 text-sm px-3 py-1"
                >
                  Cerrar Sesión
                </Button>
              </div>
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="md:hidden bg-elementz-dark/98 backdrop-blur-md border-t border-elementz-slate">
          <div className="px-4 py-4 space-y-4">
            <button
              onClick={() => handleNavigation("/")}
              className={`block font-lato ${
                isActive("/") ? "text-elementz-brown font-semibold" : "text-elementz-cream"
              }`}
            >
              Inicio
            ...
            {user ? (
              <>
                <button onClick={() => handleNavigation("/dashboard")} className="block text-elementz-gray font-lato">
                  Mi Dashboard
                </button>
                <button
                  onClick={() => {
                  ...
                  className="block text-elementz-gray font-lato"
                >
                  Cerrar Sesión
                </button>
              </>
            ) : (
              <button onClick={() => handleNavigation("/login")} className="block text-elementz-gray font-lato">
                Iniciar Sesión
              </button>
            )}
          </div>
